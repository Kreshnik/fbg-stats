...
const userPromise = FacebookGraphService.getUsers(
  `/${GROUP_ID}/members?limit=${LIMIT}`
);
userPromise
  .then(function(users) {
    const postPromise = FacebookGraphService.getPosts(
      `/${GROUP_ID}/feed?limit=${LIMIT}&fields=from`
    );
    postPromise
      .then(function(posts) {
